...
Warehouse manual storage and stock operations are integrated with Audit Core.
...
Automatic `Sale` movement from Deal completion is not audited separately. Automatic `Return` movement from DealReturn completion is not audited separately. Those flows are represented by Deals audit events and Warehouse `StockMovement` records.
