...
Проект находится на стадии готового Identity foundation/runtime, реализованных бизнес-модулей Clients и Catalog, а также MVP/Core модулей Deals MVP, Warehouse Core, Bonus Core и Returns Core inside Deals.
...
Следующий логичный модуль после Returns Core: **Chat Core with SignalR**.

Почему Chat следующий:
...
- Chat добавит коммуникационный слой CRM без изменения уже закрытого финансово-складского core flow.
...
8. `Chat Core with SignalR` - planned next
9. `Email Campaigns` - planned after Chat
10. `Audit` - planned after Chat/Email or parallel to them
11. `Minimal React frontend` - planned
12. `Advanced analytics` - planned later
13. `External integrations` and API keys - planned later

After Returns Core, implement Chat Core with SignalR next.
