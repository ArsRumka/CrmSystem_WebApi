...
Проект находится на стадии готового Identity foundation/runtime, реализованных бизнес-модулей Clients и Catalog, MVP/Core модулей Deals MVP, Warehouse Core, Bonus Core, Returns Core inside Deals, Chat Core with SignalR, Email Campaigns Core, Audit Core, а также реализованного Testing Layer / API Integration Tests.
...
- Testing Layer / API Integration Tests в `tests/CrmSystem.ApiTests`;
...
- **Audit Core** - core implemented.
- **Testing Layer / API Integration Tests** - implemented.
...
Audit Core фиксирует ключевые события Deals/Returns. Дальнейшим расширением Deals остаётся advanced analytics.
...
Audit Core реализован после Email Campaigns Core как отдельный cross-cutting module.

Реализовано:

- отдельные проекты `Audit.Domain`, `Audit.Application`, `Audit.Infrastructure`, `Audit.Presentation`;
- таблица `AuditLogs`;
- read-only API для просмотра audit logs;
- internal `IAuditLogService` для записи audit events из других модулей;
- manual audit calls в выбранных handlers;
- tenant-scoped audit logs;
- sensitive data sanitization;
- migration `AddAuditCoreModule`.
...
- миграции находятся только в `Infrastructure/Migrations`;
- EF configurations подключены через `IEfConfigurationAssemblyProvider`;
- внешних FK/navigation на Identity, Organization и business modules нет;
- audit entries сохраняются через UnitOfWork вызывающего handler;
...
Testing Layer реализован после Audit Core как отдельный слой API integration tests.

Фактически реализовано:

- 16 API integration tests;
- реальные HTTP endpoints через `HttpClient`;
- запуск WebApi через `WebApplicationFactory<Program>`;
- PostgreSQL Testcontainers;
- реальные EF migrations;
- Respawn reset между тестами;
- fake SMTP вместо реальной отправки писем;
- `EmailAutomationHostedService` отключён в тестах.

Следующий логичный этап: **minimal React frontend**.

После frontend запланированы Dockerization и Nginx reverse proxy.
...
10. `Audit Core` - core implemented, see `docs/AUDIT_MODULE.md`
11. `API integration tests` - implemented Testing Layer, see `docs/TESTING.md`
12. `Minimal React frontend` - planned next
...
After Audit Core, Testing Layer was implemented. Next: minimal React frontend, then Dockerization and Nginx reverse proxy.
...
- Warehouse return movement при completed `DealReturn`;
- Bonus refund/reversal при completed `DealReturn`;
- Audit events for deal lifecycle and returns lifecycle;
- связи с Clients, Catalog, Identity, Bonus и Warehouse через Guid ID без жёстких EF navigation/FK.

Не реализовано в Deals Core:

- advanced pipelines and analytics.

Deals MVP/Core описан в `docs/DEALS_MODULE.md`. Returns Core уже реализован внутри Deals; Audit Core фиксирует ключевые события сделок и возвратов.
...
Аудит действий. Audit Core реализован.

Реализованные Core-возможности:

- запись ключевых бизнес-действий;
- read-only API `GET /api/audit/logs` и `GET /api/audit/logs/{id}`;
- tenant scoping через `OrganizationId`;
- internal `IAuditLogService` для других модулей;
- manual audit calls без EF interceptor;
- sensitive data sanitization.
...
- Testing Layer / API Integration Tests реализован и использует PostgreSQL Testcontainers и fake SMTP.
- Следующий технический этап: minimal React frontend, затем Dockerization и Nginx reverse proxy.
