...
Проект находится на стадии готового Identity foundation/runtime, реализованных бизнес-модулей Clients и Catalog, а также MVP/Core модулей Deals MVP, Warehouse Core, Bonus Core, Returns Core inside Deals и Chat Core with SignalR.
...
Следующий логичный модуль после Chat Core with SignalR: **Email Campaigns Core**.

Почему Email Campaigns следующий:
...
- Chat уже добавил коммуникационный слой CRM без изменения финансово-складского core flow.
- Email Campaigns расширит коммуникационные сценарии от диалогов к массовым клиентским рассылкам.
- Audit лучше делать после Email Campaigns, чтобы логировать ключевые действия коммуникационных модулей и основных business flows.
...
9. `Email Campaigns Core` - planned next
10. `Audit` - planned after Email Campaigns
11. `Minimal React frontend` - planned
12. `Advanced analytics` - planned later
13. `External integrations` and API keys - planned later

After Chat Core with SignalR, implement Email Campaigns Core next.
...
Клиентские email-рассылки. Planned next после Chat Core with SignalR.

Ожидаемые Core-возможности:

- кампании рассылок;
- выбор клиентов по базовым фильтрам;
- шаблон письма;
- ручной запуск кампании;
- статус отправки;
- связь с Clients через Guid ID без внешних FK.

Email Campaigns лучше реализовывать до Audit, чтобы затем логировать действия коммуникационных модулей целиком: Chat и Email Campaigns.
